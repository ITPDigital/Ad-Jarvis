...
window.onload = (event) => {
	...
	if(jQuery('[data-adtype="banner300x600"]').hasClass("ad-home")) {
    ...
    else if(jQuery('body').hasClass('archive') || jQuery('body').hasClass('single-magazine-issue')) {
		jQuery('#secondary > section:first-child').stickit({
			top: 190,
			screenMinWidth: 1078
		});
	}
		...
		if(jQuery('.main-content').height() >= jQuery('aside#secondary').height()) {
			jQuery('#secondary > section:first-child').stickit('destroy');
			jQuery('#secondary > section:first-child').stickit({
				top: 190,
				screenMinWidth: 1078
			});
