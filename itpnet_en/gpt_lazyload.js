    ...
    googletag.cmd.push(function() {
        if (!isRenderEventFired) {
            ...
            googletag.pubads().addEventListener('slotOnload', function(event) {
                ...
                if (!event.isEmpty) {
                    if (renderedAdType == "Leaderboard_ATF") { //BB block
                        if (enableStickyAds && !isBBSlotViewed) {
                            ...
                            jQuery(window).scroll(function() { //sticky bb if ad is not viewed for bbViewTime
                                //	console.log("----------Top banner viewed for----------"+x+"seconds");
								 ...
								 var offsetTop = jQuery('.code-block-3').height();
							     if(jQuery('body').hasClass('single-events')) {
								   offsetTop = jQuery('.top-block').height();
							     }
								 if(offsetTop) {
									 offsetTop = offsetTop+81;//adding header height
								 }
                    ...
                    } else if (renderedAdType == 'MPU_MID') {
						/*if(!jQuery('[data-adtype="MPU_MID"]').hasClass("ad-home")) {
							jQuery('[data-adtype="MPU_MID"]').parent().stickit('destroy');
							jQuery('[data-adtype="MPU_MID"]').parent().stickit({
								top: 135,
								screenMinWidth: 768
							});
						}*/
                    } else if (renderedAdType == 'MPU_MID_HPB') {
						/*jQuery('[data-adtype="MPU_MID_HPB"]').parent().stickit('destroy');
							jQuery('[data-adtype="MPU_MID_HPB"]').parent().stickit({
								top: 135,
								screenMinWidth: 768
							});*/
                    } else if (renderedAdType == 'HalfPage_ATF') {
                        /*jQuery('[data-adtype="HalfPage_ATF"]').parent().stickit('destroy');
                        jQuery('[data-adtype="HalfPage_ATF"]').parent().stickit({
                            top: 135,
                            screenMinWidth: 768
                        });*/
                    }
...
window.onload = (event) => {
	...
	if(jQuery('[data-adtype="HalfPage_ATF"]').hasClass("ad-home")) {
		jQuery('[data-adtype="HalfPage_ATF"]').parent().stickit({
			top: 135,
			screenMinWidth: 768
		});
	} else if(jQuery('body').hasClass('archive')) {
		jQuery('#secondary > div:first-child').stickit({
			top: 135,
			screenMinWidth: 782
		});
	}
	else if(jQuery('body').hasClass('single')) {
		if(jQuery('.main-content').height() >= jQuery('aside#secondary').height()) {
			jQuery('#secondary > div:first-child').stickit('destroy');
			jQuery('#secondary > div:first-child').stickit({
				top: 135,
				screenMinWidth: 901
			});
