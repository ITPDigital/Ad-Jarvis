                ...
                if (event.isEmpty) {
                    ...
                    if (renderedAdType == "banner750x450") { //intertials ad
						...
						if(jQuery(id).length > 0) {
							...
							elem.parentElement.parentElement.classList.add("activated");
            ...
            googletag.pubads().addEventListener('slotOnload', function(event) {
                ...
                if (!event.isEmpty) {
                    if (renderedAdType == "banner970x250") { //BB block
						...
						if(jQuery('[data-adtype="banner300x250"]').hasClass('ad-home')) {
						...
						if(jQuery('[data-adtype="banner300x600"]').hasClass('ad-home')) {
...
function constructOverlay(elem, event) {
	console.log("before constructOverlay");
    elem.closest(".googleAds").style.height = "";
    ...
    var popOutWrap = document.querySelector('.popWrapper');
    popOutWrap.style.width = event.size[0] + 'px';/*comment it for full-screen overlay*/
    popOutWrap.style.height = event.size[1] + 'px';/*comment it for full-screen overlay*/
   /* popOutWrap.style.marginLeft = '-' + (event.size[0] / 2) + 'px';
    popOutWrap.style.marginTop = '-' + (event.size[1] / 2) + 'px';*/
    //timer on
	console.log("before elcose");
    var secOver = 8;
    var interval = setInterval(function() {
        secOver = secOver - 1;
        if (secOver == -2) {
			console.log("insid elcose");
            bannerCloseOverlay();
            clearInterval(interval);
        }
    }, 1000);
...
window.onload = (event) => {
	...
	if(jQuery('body').hasClass('archive') || jQuery('body').hasClass('single-magazine-issue')) {
		jQuery('#secondary > div:first-child').stickit({
			top: 135,
			screenMinWidth: 1078
		});
	}
	else if(jQuery('body').hasClass('has-sidebar') && jQuery('body').hasClass('page')) {
		jQuery('#secondary > div:first-child').stickit({
			top: 132,
			screenMinWidth: 992
		});
