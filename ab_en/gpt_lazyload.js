            ...
            googletag.pubads().addEventListener('slotRenderEnded', function(event) {
                ...
                if (event.isEmpty) {
                    ...
                    if (renderedAdType == "banner450x450") { //overlay ad
						...
						console.log("*************here***");
						if(jQuery('[data-adtype="banner300x250"]').hasClass('cat-ad')) {
							console.log("*************inside here***");
							elem.parentElement.parentElement.classList.remove("ad-loaded");
							elem.parentElement.parentElement.classList.add("no-ad-loaded");
						}
                    } else if (renderedAdType == 'banner320x50') {
						jQuery('body').removeClass('mob-lb-loaded');
                    }
                } else {
                    ...
                    if (renderedAdType == "banner750x450") { //intertials ad
						...
						if(jQuery('[data-adtype="banner300x250"]').hasClass('cat-ad')) {
							console.log("*************inside here***");
							elem.parentElement.parentElement.classList.add("ad-loaded");
						}
                    } else if (renderedAdType == 'banner320x50') {
						jQuery('body').addClass('mob-lb-loaded');
                    }
                }
            });
            ...
            googletag.pubads().addEventListener('slotOnload', function(event) {
                ...
                if (!event.isEmpty) {
                    if (renderedAdType == "banner970x250") { //BB block
						...
						/*if(jQuery('[data-adtype="banner300x250"]').hasClass('list-ads')) {
							if(jQuery('body').hasClass('single')) {
								if(jQuery('.main-content-inr').height() > jQuery('aside#secondary').height()) {
									jQuery('.sidebar-ctr').stickit('destroy');
									jQuery('.sidebar-ctr').stickit({
										top: 135,
										screenMinWidth: 992
									});
								}
							}  else {
								jQuery('#block-15').stickit('destroy');
								jQuery('#block-15').stickit({
									top: 135,
									screenMinWidth: 768
								});
							}
						}
						if(jQuery('[data-adtype="banner300x250"]').hasClass("comb-ad") && !jQuery('[data-adtype="banner300x250"]').hasClass('ad-home')) {//not for home page
						...
						/*if(jQuery('[data-adtype="banner300x600"]').hasClass('list-ads')) {
							if(jQuery('body').hasClass('single')) {
								if(jQuery('.main-content-inr').height() > jQuery('aside#secondary').height()) {
									jQuery('.sidebar-ctr').stickit('destroy');
									jQuery('.sidebar-ctr').stickit({
						...
						if(jQuery('[data-adtype="banner300x600"]').parent() && !jQuery('[data-adtype="banner300x600"]').hasClass('ad-home')) {
    ...
    if (!elem.parentElement.classList.contains("intertials_block")) {
        ...
        var interval = setInterval(function() {
            ...
            if (sec > 0) {
                ...
                wrap2.innerHTML = "<a id='redirectlink' class='bannerClose continue-color' style='color:#000;'>Continue to Site »</a>";
                ...
                for (var j = 0; j < adSlotArr.length; j++) {
                    if (adSlotArr[j].name == gptAdtype) {
                        ...
                        if (!isSingleElement) {
							...
							if(slotIndex == 'auto') {
								slotIndex = generateRandomId();
								googleAdsList[i].dataset.index = slotIndex;
								googleAdsList[i].id = "ad-slot-"+slotIndex;
							}
                        }
...
window.onload = (event) => {
	...
	/*if(jQuery('body').hasClass('home')) {
		jQuery('.home .hb-1 .hsb-4').stickit('destroy');
		jQuery('.home .hb-1 .hsb-4').stickit({
			top: 135,
			screenMinWidth: 768
		});
	}*/
	if(jQuery('body').hasClass('single-magazine-issue') || jQuery('body').hasClass('category-magazine') || jQuery('body').hasClass('category-lists') || jQuery('body').hasClass('tax-list') || jQuery('body').hasClass('search')) {
		jQuery('#block-15').stickit({
			top: 135,
			screenMinWidth: 768
		});
		...
		setTimeout(function(){
			if(jQuery('#ev-meter-content3').length>0 || jQuery('#ev-meter-content6').length>0) {
				jQuery('.sidebar-ctr').stickit('destroy');
			}
		}, 4000);
	}
