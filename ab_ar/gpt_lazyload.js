...
function constructInterstitialPopup(elem) {
    ...
    if (!elem.parentElement.classList.contains("intertials_block")) {
        ...
        var interval = setInterval(function() {
            sec = sec - 1;
            if (sec > 0) {
                wrap2.innerHTML = "بإمكانكم تخطي هذا الإعلان بعد عدة ثوان " + sec;
                //wrap2.style.color = '#000';
            } else if (sec == 0) {
                wrap2.innerHTML = "<a id='redirectlink' class='bannerClose continue-color' style='color:#000;'>المتابعة إلى الموقع »</a>";
...
window.onload = (event) => {
	...
	if(jQuery('body').hasClass('archive')) {
		jQuery('#secondary > .widget_block:last-child').stickit({
			top: 135,
			screenMinWidth: 768
		});
	}
