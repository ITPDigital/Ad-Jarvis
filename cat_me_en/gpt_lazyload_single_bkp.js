...
if(!isRenderEventFired) {
	...
	googletag.pubads().addEventListener('slotRenderEnded', function(event) {
		...
		if (event.isEmpty) {
			...
			if(renderedAdType == "banner300x250") {//mpu
				if(elem.parentElement.parentElement.classList.contains("cus-ad-col")){
					elem.parentElement.parentElement.classList.add("hide");
					elem.parentElement.parentElement.parentElement.classList.add("show-last-child");
				}
			}
			else if(renderedAdType == "banner970x250") {//BB
				//jQuery(".header__region").removeClass("ad-fixed");
			}
			else if(renderedAdType == "banner_450X450") { //overlay ad
				elem.classList.add("banner_450X450")
			...
			else if(renderedAdType == "banner300x250") {//mpu
				if(elem.parentElement.parentElement.classList.contains("cus-ad-col")){
					elem.parentElement.parentElement.classList.remove("hide");
					elem.parentElement.parentElement.parentElement.classList.remove("show-last-child");
				}
			}
			else if(renderedAdType == "banner_450X450") { //overlay ad
					...
					if(renderedAdType == "banner970x250") {//BB block
						if(enableStickyAds && !isBBSlotViewed) {
							...
							//	console.log("----------Top banner viewed for----------"+x+"seconds");
								...
								else if(jQuery(window).scrollTop()>=200 && !isScrolled && x<bbViewTime) {
									clearInterval(topAdTimer);
									isScrolled = true;
									setTimeout(function(){
										if(jQuery(window).scrollTop()>=200) {
											console.log("************Add inside ad-fixed***********"+jQuery(window).scrollTop()+"******"+(jQuery(window).scrollTop()>=200));
											isBBSlotViewed = true;
						...
						jQuery('[data-adtype="banner300x250"]').parent().stickit({top: 50,screenMinWidth: 1280});
					}
					else if ( renderedAdType == 'banner300x600') {
						jQuery('[data-adtype="banner300x600"]').parent().stickit('destroy');
						jQuery('[data-adtype="banner300x600"]').parent().stickit({top: 50,screenMinWidth: 1280});
					}
				}
			});
 }
  ...
  if(slotIndex) {
	  console.log("----slotIndex---"+gptAdtype);
	  isElemFiftyPercInViewport(currElem,slotIndex,gptAdtype);
  }
}
...
function constructInterstitialPopup(elem) {
	if(!elem.parentElement.classList.contains("intertials_block") && cookieVal!= "skipAdLoaded") {
		...
		wrap1.style.backgroundColor = '#d4b57b';
...
function constructCatfish(elem, event) {
	elem.closest(".ad-slot").style.height = "";
	elem.closest(".ad-slot").classList.add("catfish_block");
...
function constructOverlay(elem, event) {
	elem.closest(".ad-slot").style.height = "";
	elem.closest(".ad-slot").classList.add("overlay_block");
	var popOutWrap = document.querySelector('.popWrapper');
	popOutWrap.style.width = event.size[0] + 'px';
	popOutWrap.style.height = event.size[1] + 'px';
	popOutWrap.style.marginLeft = '-' + (event.size[0] / 2) + 'px';
...
function defineAdSlots(slotIndex) {
	...
	if(slotIndex) {
		...
		googleAdsList = document.querySelectorAll('.ad-slot:not([class*="more-ad"])');
	}
				...
				if(isValidSlot(gptAdtype,googleAdsList[i])) {
					for (var j = 0; j < adSlotArr.length; j++) {
						if(adSlotArr[j].name == gptAdtype) {
							...
							if(gptAdtype == "banner_450X450") {//overlay
...
function getGPTSizeMapping(adArr) {
	return googletag.sizeMapping()
	  .addSize([768, 0], adArr.primarySlot)
	...
	if(!elemInView) {
		document.addEventListener('scroll', function(e) {
			if(!elemInView) {
				...
				googletag.cmd.push(function() { googletag.display(getPositionName(slotIndex));});
			}
		});
	} else {
		googletag.cmd.push(function() { googletag.display(getPositionName(slotIndex));});
